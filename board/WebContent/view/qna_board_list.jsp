				...
				<nav aria-label="Page navigation example">
				  ...
				  <c:forEach begin="${info.startPage}" end="${info.endPage}" var="idx">
				  		<c:choose>
				  			...
				  			<c:otherwise>
				  			<li class="page-item"><a class="page-link" href="qList.do?page=${info.search.page}&criteria=${info.search.criteria}&keyword=${info.search.keyword}">${idx}</a></li>
				  			</c:otherwise>
				  		</c:choose>
				  </c:forEach>
				  <c:if test="${info.next}">
...
<script>
$(function(){
	//input type=text는 입력후 엔터키가 자동실행되기 때문에 엔터치는것 막기 keydown
	
	$("[name='keyword']").keydown(function(e){
		if(e.keyCode == 13){
			e.preventDefault(); //submit,a,text의 엔터 기능가지고 있는 태그 막기
		}
	})
	//검색버튼을 클릭하면
